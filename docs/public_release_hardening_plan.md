...
| PHR-03 | Release/citation metadata are stale. | `pyproject.toml` and `CITATION.cff` reported a pre-1.0 placeholder version, placeholder author, and example repository URL; `LICENSE` said contributors. | verified | high | `pyproject.toml`, `CITATION.cff`, `LICENSE` | Align metadata to Ansh Hemang Dani, v1.0.0, real repository URL, and planned release date. | Low | yes | version consistency tests |
...
## Public Metadata Normalization Note

Tool-specific development metadata was normalized for the public v1.0.0 tree. Original historical values remain preserved in prior Git commits. No experimental output, label, metric, or scientific conclusion was changed.
